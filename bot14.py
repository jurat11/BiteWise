...
# Updated Translations with New Recommendation Keys
TRANSLATIONS = {
    "intro": {
    ...
    "recommendation_lose_weight": {
        "en": "To lose weight, aim to consume around {calories} calories per day.\nRecommended daily intake:\n- Protein: {protein}g\n- Carbs: {carbs}g\n- Fat: {fat}g",
        "ru": "Чтобы похудеть, стремитесь потреблять около {calories} калорий в день.\nРекомендуемая дневная норма:\n- Белки: {protein}г\n- Углеводы: {carbs}г\n- Жиры: {fat}г",
        "uz": "Vazn kamaytirish uchun kuniga taxminan {calories} kaloriya iste'mol qiling.\nKunlik tavsiya etilgan miqdor:\n- Oqsil: {protein}g\n- Uglevodlar: {carbs}g\n- Yog'lar: {fat}g"
    },
    "recommendation_gain_muscle": {
        "en": "To gain muscle, aim to consume around {calories} calories per day.\nRecommended daily intake:\n- Protein: {protein}g\n- Carbs: {carbs}g\n- Fat: {fat}g",
        "ru": "Чтобы набрать мышечную массу, стремитесь потреблять около {calories} калорий в день.\nРекомендуемая дневная норма:\n- Белки: {protein}г\n- Углеводы: {carbs}г\n- Жиры: {fat}г",
        "uz": "Massa oshirish uchun kuniga taxminan {calories} kaloriya iste'mol qiling.\nKunlik tavsiya etilgan miqdor:\n- Oqsil: {protein}g\n- Uglevodlar: {carbs}g\n- Yog'lar: {fat}g"
    },
    "recommendation_maintain": {
        "en": "To maintain your weight, aim to consume around {calories} calories per day.\nRecommended daily intake:\n- Protein: {protein}g\n- Carbs: {carbs}g\n- Fat: {fat}g",
        "ru": "Чтобы поддерживать вес, стремитесь потреблять около {calories} калорий в день.\nРекомендуемая дневная норма:\n- Белки: {protein}г\n- Углеводы: {carbs}г\n- Жиры: {fat}г",
        "uz": "Vazningizni saqlab qolish uchun kuniga taxminan {calories} kaloriya iste'mol qiling.\nKunlik tavsiya etilgan miqdor:\n- Oqsil: {protein}g\n- Uglevodlar: {carbs}g\n- Yog'lar: {fat}g"
    },
    "already_registered": {
    ...
    "nutrition_terms": {
        "en": {"calories": "Calories", "protein": "Protein", "carbs": "Carbs", "fat": "Fat", "sodium": "Sodium", "fiber": "Fiber", "sugar": "Sugar"},
        "ru": {"calories": "Калории", "protein": "Белок", "carbs": "Углеводы", "fat": "Жиры", "sodium": "Натрий", "fiber": "Клетчатка", "sugar": "Сахар"},
        "uz": {"calories": "Kaloriyasi", "protein": "Oqsil", "carbs": "Uglevodlar", "fat": "Yog'lar", "sodium": "Natriy", "fiber": "Tola", "sugar": "Shakar"}
    },
    "disclaimer": {
...
def get_meal_action_keyboard(lang: str, meal_id: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ OK", callback_data="meal_ack")]
    ])
...
def calculate_daily_requirements(user_data: dict) -> dict:
    ...
    # Macronutrient ratios (research-based)
    protein_ratio = 0.25  # 25% (1.0-1.6g/kg, midpoint for general use)
    carbs_ratio = 0.50    # 50%
    fat_ratio = 0.25      # 25%

    # Grams per calorie
    protein_grams = (daily_calories * protein_ratio) / 4
    carbs_grams = (daily_calories * carbs_ratio) / 4
    fat_grams = (daily_calories * fat_ratio) / 9

    # Sodium, fiber, sugar limits (mg or g)
    sodium_limit = 2300
    fiber_goal = 30  # General recommendation
    sugar_limit = 50  # WHO recommendation (~10% of calories)

    return {
        'daily_calories': round(daily_calories, 2),
        'daily_protein': round(protein_grams, 2),
        'daily_carbs': round(carbs_grams, 2),
        'daily_fat': round(fat_grams, 2),
        'daily_sodium': sodium_limit,
        'daily_fiber': fiber_goal,
        'daily_sugar': sugar_limit
    }

def get_recommendation(user_data: dict, lang: str) -> str:
    goal = user_data.get('goal', '').lower()
    daily_calories = user_data['daily_calories']
    protein = user_data['daily_protein']
    carbs = user_data['daily_carbs']
    fat = user_data['daily_fat']

    if "lose_weight" in goal or "vazn kamaytirish" in goal:
        return t("recommendation_lose_weight", lang, calories=daily_calories, protein=protein, carbs=carbs, fat=fat)
    elif "gain_muscle" in goal or "massa oshirish" in goal:
        return t("recommendation_gain_muscle", lang, calories=daily_calories, protein=protein, carbs=carbs, fat=fat)
    else:
        return t("recommendation_maintain", lang, calories=daily_calories, protein=protein, carbs=carbs, fat=fat)
...
async def send_motivational_quote(user_id: int):
    try:
        ...
        advice = ""
        if "lose_weight" in goal.lower() or "vazn kamaytirish" in goal.lower():
        ...
        elif "gain_muscle" in goal.lower() or "massa oshirish" in goal.lower():
        ...
        elif "eat_healthier" in goal.lower() or "sog'lom ovqat" in goal.lower():
            advice = t("eat_healthier", lang) + ": Focus on whole foods. "
        elif "look_younger" in goal.lower() or "yoshroq ko'rinish" in goal.lower():
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if user_ref.get().exists:
        ...
        await state.clear()
        intro_message = f"{t('intro', 'en')}\n{t('intro', 'ru')}\n{t('intro', 'uz')}"
        await message.answer(intro_message, reply_markup=get_language_inline_keyboard())
...
@dp.callback_query(F.data == "meal_ack")
async def handle_meal_ack(callback: types.CallbackQuery):
    await callback.answer()
...
async def process_goal_selection(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        await bot.send_message(
            ...
            "\n\nSend me food info or photos to get started!",
            reply_markup=get_main_menu_keyboard(lang)
        )
        await state.clear()
    except Exception as e:
...
async def show_stats(message: types.Message):
    try:
        ...
        water_bar = "🟦" * min(int((today_water / daily_water_goal) * progress_bar_length), progress_bar_length) + "⬜" * (progress_bar_length - min(int((today_water / daily_water_goal) * progress_bar_length), progress_bar_length))
        ...
        response = (
            ...
            f"⚡ <b>Calories:</b> {today_calories} / {daily_calories} kcal ({calories_percent}%)\n"
            f"💪 <b>Protein:</b> {today_protein} / {daily_protein}g ({protein_percent}%)\n"
            f"🍚 <b>Carbs:</b> {today_carbs} / {daily_carbs}g ({carbs_percent}%)\n"
            f"🧈 <b>Fat:</b> {today_fat} / {daily_fat}g ({fat_percent}%)\n"
            f"🧂 <b>Sodium:</b> {today_sodium} / {daily_sodium}mg ({sodium_percent}%)\n"
            f"🌾 <b>Fiber:</b> {today_fiber} / {daily_fiber}g ({fiber_percent}%)\n"
            f"🍬 <b>Sugar:</b> {today_sugar} / {daily_sugar}g ({sugar_percent}%)\n\n"
        ...
        if 'height' in user_data and 'weight' in user_data:
            ...
            bmi_categories = {
                ...
                "uz": {"underweight": "Vazn yetishmovchiligi", "normal": "Normal vazn", "overweight": "Ortiqcha vazn", "obesity": "Semizlik"}
            }
            category = "normal"
            if bmi < 18.5:
