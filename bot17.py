...
def get_recommendation(user_data: dict, lang: str) -> str:
    ...
    logger.info(f"Generating recommendation for goal: {goal}")
...
async def send_motivational_quote(user_id: int):
    try:
        ...
        if goal == "lose_weight":
            ...
            if today_calories > user_data.get('daily_calories', 2000):
            ...
            if today_calories < user_data.get('daily_calories', 2500):
        ...
        prompt = (f"Generate a 2-3 sentence motivational message in {lang} "
                  f"addressed to {user_name}, who has the goal: {t(goal, lang)}. "
...
async def process_goal_selection(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        # Add validation for goal keys
        valid_goals = ["lose_weight", "gain_muscle", "eat_healthier", "look_younger", "other_goal"]
        if goal_key not in valid_goals:
            raise ValueError("Invalid goal key")
