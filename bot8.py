...
async def schedule_default_reminders(user_id: int, timezone: str):
    """
    Schedule all reminders for a user at their proper times.
    - Water: 08:00, 10:30, 13:00, 15:30, 18:00, 20:30
    - Meals: Breakfast 08:30, Lunch 13:00, Dinner 19:00
    - Motivation: 21:00
    - Weekly summary: Sunday 20:00
    - Weight update: Sunday 18:00 (if goal is lose_weight)
    """
    try:
        ...
        if user_data.get('water_reminders', True):
            ...
            for hour, minute in water_times:
                ...
                logger.info(f"Scheduled water reminder for user {user_id} at {hour:02d}:{minute:02d}")
        ...
        if user_data.get('meal_reminders', True):
            ...
            for time, meal_type in meal_times:
                ...
                logger.info(f"Scheduled {meal_type} reminder for user {user_id} at {hour:02d}:{minute:02d}")
        ...
        if user_data.get('motivational_quotes_enabled', True):
            ...
            logger.info(f"Scheduled motivational quote for user {user_id} at 21:00")
        ...
        if user_data.get('breakfast_reminder_enabled', True):
            ...
            logger.info(f"Scheduled breakfast reminder for user {user_id} at 08:30")
        ...
        if user_data.get('lunch_reminder_enabled', True):
            ...
            logger.info(f"Scheduled lunch reminder for user {user_id} at 13:00")
        ...
        if user_data.get('dinner_reminder_enabled', True):
            ...
            logger.info(f"Scheduled dinner reminder for user {user_id} at 19:00")
        ...
        logger.info(f"Scheduled weekly summary for user {user_id} at Sunday 20:00")
        ...
        if user_data.get('goal') == "lose_weight":
            ...
            logger.info(f"Scheduled weight update prompt for user {user_id} at Sunday 18:00")

        logger.info(f"Scheduled ALL reminders for user {user_id} (normal times)")
    except Exception as e:
...
async def send_meal_reminder(user_id: int, meal_type: str):
    try:
        ...
        # No inline keyboard for meal reminders
...
async def send_motivational_quote(user_id: int):
    try:
        ...
        # --- Daily-changing prompt: use day of year as seed ---
        ...
        # --- Personalized, 2-sentence, language-aware prompt ---
        prompt = (
...
async def main():
    scheduler.start()
    # Schedule reminders for all users on startup (async, with small delay)
    all_users = list(db.collection('users').stream())
    for user_doc in all_users:
        ...
        try:
            asyncio.create_task(schedule_default_reminders(user_id, timezone))
            logger.info(f"Scheduled reminders for user {user_id} on startup (async).")
        except Exception as e:
