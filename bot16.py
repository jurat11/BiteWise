...
# Configuration
BOT_TOKEN = "8027102621:AAG7UiDBfywFMgBIKDyrgRJ7hFG2PBgkJmI"  # Replace with your actual bot token
...
ADMIN_ID = 5080813917
BOT_USERNAME = "@KinoMania4k_Bot"  # Replace with your bot's username (without @)
...
TRANSLATIONS = {
    ...
    "ask_custom_water": {"en": "Enter water amount in ml (0-5000):", "ru": "Введите количество воды в мл (0-5000):",
                         "uz": "Suv miqdorini ml da kiriting (0-5000):"},
    "water_logged_custom": {"en": "✅ Water intake recorded! +{amount}ml", "ru": "✅ Вода записана! +{amount}мл",
                            "uz": "✅ Suv qayd etildi! +{amount}ml"},
    "water_amount_error": {"en": "Please enter a valid amount between 0 and 5000 ml.",
                           "ru": "Пожалуйста, введите правильное количество от 0 до 5000 мл.",
                           "uz": "Iltimos, 0 dan 5000 ml gacha to'g'ri miqdorni kiriting."}
}
...
async def schedule_default_reminders(user_id: int, timezone: str):
    try:
        ...
        if all([water_disabled, meal_disabled, motivation_disabled, breakfast_disabled, lunch_disabled,
                dinner_disabled]):
        ...
        logger.info(
            f"Scheduled ALL reminders for user {user_id} (timezone: {timezone}). Total jobs now: {len(scheduler.get_jobs())}")
    except Exception as e:
...
async def handle_water_option(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "water_custom":
        ...
        asyncio.create_task(update_streaks_and_challenges(user_id, "water"))
        await callback.message.answer(t("water_logged_custom", lang, amount=amount),
                                      reply_markup=get_main_menu_keyboard(lang))
        await callback.answer()
...
@dp.message(lambda m: m.text and (
        m.text.strip().lower() == "/admin" or m.text.strip().lower() == f"/admin@{BOT_USERNAME.lower()}"),
            StateFilter(None))
@dp.message(lambda m: m.text and (
        m.text.strip().lower() == "/admin" or m.text.strip().lower() == f"/admin@{BOT_USERNAME.lower()}"), state='*')
