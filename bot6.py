...
def get_main_menu_keyboard(lang: str) -> ReplyKeyboardMarkup:
    keyboard = [
        ...
        [KeyboardButton(text=t("menu_help", lang)), KeyboardButton(text=t("menu_donate", lang))]
    ]
...
async def get_user_language(user_id: int) -> str:
    ...
    logger.info(f"User {user_id} language retrieved: {lang}")
    return lang
...
async def schedule_default_reminders(user_id: int, timezone: str):
    logger.info(f"Scheduled water reminder for user {user_id} at 10:00 {timezone}")
    user_ref = get_user_ref(user_id)
    user_data = user_ref.get().to_dict()
    lang = await get_user_language(user_id)
...
async def process_new_language_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await bot.send_message(user_id, t("main_menu", updated_lang), reply_markup=get_main_menu_keyboard(updated_lang))
...
async def edit_reminders_callback(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        keyboard = []
        for reminder_type in ["water", "meal", "motivational"]:
            is_enabled = user_data.get(f"{reminder_type}_reminders_enabled", True)
            status = "✅" if is_enabled else "❌"
            text = t(f"{reminder_type}_reminders", lang)
            keyboard.append([InlineKeyboardButton(
                text=f"{text} {status}",
                callback_data=f"toggle_{reminder_type}_reminders"
            )])
...
async def handle_reminder_toggle(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action in ["toggle_water", "toggle_meal", "toggle_motivation"]:
...
async def process_meal_text(message: types.Message, state: FSMContext):
    try:
        ...
        # Matn va rasmni olish
        text = message.text
        image_content = None

        if message.photo:
            photo = message.photo[-1]
            file_size = photo.file_size  # Rasm hajmini olish
            logger.info(f"User {user_id} sent photo, size: {file_size} bytes, text: '{text}'")
            try:
                ...
                await message.answer(t("image_processing_error", lang))
                await state.clear()
                return
            if not text:
                text = ""
        ...
        try:
            ...
            if not analysis.get('analysis_successful', False):
                await message.answer(t("food_not_recognized", lang))
                await state.clear()
                return

            # Ovqat ma'lumotlarini saqlash
            ...
            # Qayta ishlash xabarini o'chirish
            try:
                await processing_msg.delete()
            except:
                pass

            # Oziqlanish ma'lumotlarini tayyorlash
            nutrition_terms = t('nutrition_terms', lang)
            daily_req = t('daily_requirement', lang).format(percentage=analysis['calories']['percentage'])
            nutrition_info = [
                f"🍽 {t('food_analysis_header', lang).format(food_name=analysis['name'])}",
                "",
                f"- {nutrition_terms['calories']}: {analysis['calories']['amount']} kcal {daily_req} 🧮",
                f"- {nutrition_terms['protein']}: {analysis['protein']['amount']}g 💪",
                f"- {nutrition_terms['carbs']}: {analysis['carbs']['amount']}g 🍚",
                f"- {nutrition_terms['fat']}: {analysis['fat']['amount']}g 🧈",
                f"- {nutrition_terms['sodium']}: {analysis['sodium']['amount']}mg 🧂",
                f"- {nutrition_terms['fiber']}: {analysis['fiber']['amount']}g 🌾",
                f"- {nutrition_terms['sugar']}: {analysis['sugar']['amount']}g 🍬",
                "",
                f"⚠️ {t('disclaimer', lang)}"
            ]
