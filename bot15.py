...
async def schedule_default_reminders(user_id: int, timezone: str):
    try:
        user_data = get_user_ref(user_id).get().to_dict()
        # Only schedule for active users
        if user_data.get('active', True) is False:
            logger.info(f"Skipping scheduling for inactive user {user_id}")
            return
        ...
        if user_data.get('meal_reminders', True):
            meal_times = [("08:30", "breakfast"), ("22:17", "lunch"), ("22:12", "dinner")]
        ...
        if user_data.get('motivational_quotes_enabled', True):
            ...
            logger.info(f"Scheduled motivational quote for user {user_id} at 21:0 {timezone}")
        ...
        if user_data.get('lunch_reminder_enabled', True):
            ...
            logger.info(f"Scheduled lunch reminder for user {user_id} at 22:17 {timezone}")
        if user_data.get('dinner_reminder_enabled', True):
            ...
            logger.info(f"Scheduled dinner reminder for user {user_id} at 22:12 {timezone}")
        # Weekly summary
        scheduler.add_job(
            send_weekly_summary,
            'cron',
