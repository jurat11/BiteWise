...
async def _log_water_internal(user_id: int, lang: str) -> str:
    try:
        ...
        water_mute_until = user_data.get('water_mute_until')
        if water_mute_until and now < water_mute_until:
            return "muted"
        # Check frequent logging
        one_minute_ago = now - timedelta(minutes=1)
        water_ref = get_water_ref(user_id)
        recent_logs = list(water_ref.where('timestamp', '>=', one_minute_ago).stream())
        if len(recent_logs) >= 10:
            mute_until = now + timedelta(minutes=5)
            get_user_ref(user_id).update({'water_mute_until': mute_until})
            return "excessive"
        # Check daily limit
...
async def log_water(message: types.Message):
    try:
        ...
        if result == "muted":
            pass  # Do nothing, silently ignore
        elif result == "excessive":
            await message.answer(t("excessive_water_logging", lang))
            await message.answer(t("mute_message", lang))
...
async def callback_log_water(callback_query: types.CallbackQuery):
    try:
        ...
        if result == "muted":
            ...
            await bot.send_message(user_id, t("excessive_water_logging", lang))
            await bot.send_message(user_id, t("mute_message", lang))
        elif result == "overhydration":
...
async def handle_photo(message: types.Message, state: FSMContext = None):
    ...
    try:
        ...
        prompt += f"[Identified food name] uchun oziqaviy qiymat:\n\n"
        prompt += f"- {terms['calories']}: [value] kcal (kunlik ehtiyojning ~[percentage]%) 🧮\n"
        prompt += f"- {terms['protein']}: [value]g 💪\n"
        prompt += f"- {terms['carbs']}: [value]g 🍚\n"
        prompt += f"- {terms['fat']}: [value]g 🧈\n"
        prompt += f"- Natriy: [value]mg 🧂\n\n"
        ...
        prompt += "Assume daily calorie need is 2000 kcal. If the food is not recognized, suggest a similar common food item.\n"
