...
TRANSLATIONS = {
    ...
    "mute_message": {
        "en": "You have been muted for 5 minutes due to frequent water logging. Please try again later.",
        "ru": "Вы были отключены на 5 минут из-за частой записи воды. Пожалуйста, попробуйте позже.",
        "uz": "Siz suvni tez-tez yozganingiz uchun 5 daqiqa davomida o'chirildingiz. Iltimos, keyinroq qayta urinib ko'ring."
    }
}
...
async def _log_water_internal(user_id: int, lang: str) -> str:
    try:
        ...
        if today_water_docs:
            ...
            if (now - most_recent['timestamp']).total_seconds() < 300:  # 5 minutes
