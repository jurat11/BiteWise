...
def meal_logging_conversation() -> ConversationHandler:
    ...
    return ConversationHandler(
        ...
        states={
            AWAIT_MEAL_TYPE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex(cancel_pattern),
                               await_meal_type_handler),
                CallbackQueryHandler(await_meal_type_handler, pattern="^meal_type_.*$|^cancel_meal_log$")
            ],
            AWAIT_MEAL_INPUT: [
        ...
        fallbacks=[
            ...
            CallbackQueryHandler(cancel_meal_log_conv, pattern="^cancel_meal_log$")
        ],
        name="meal_logging",
        persistent=True
    )
...
async def stats_command_or_button(update: Update, context: CallbackContext) -> None:
    ...
    calories_goal = user_data.get('calories_goal', 2000)  # Default calories goal
    calories_today = sum(
        entry.get('calories', 0) for entry in user_data.get('food_log', {}).get(today_local.strftime('%Y-%m-%d'), []))
