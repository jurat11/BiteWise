...
def get_goals_keyboard(lang: str):
    # Custom button configuration for Uzbek
    uz_buttons = [
...
def parse_nutrition(analysis: str) -> dict:
    ...
    nutrient_lines = [line for line in lines if line.strip().startswith('-')]
    if len(nutrient_lines) >= 4:
        try:
            nutrition["calories"] = int(re.search(r'\d+', nutrient_lines[0]).group())
            nutrition["protein"] = int(re.search(r'\d+', nutrient_lines[1]).group())
            nutrition["carbs"] = int(re.search(r'\d+', nutrient_lines[2]).group())
            nutrition["fat"] = int(re.search(r'\d+', nutrient_lines[3]).group())
...
async def send_water_reminder(user_id: int):
    try:
        ...
        now = datetime.now(tz)
        time_window = now - timedelta(hours=2, minutes=30)
...
async def send_motivational_quote(user_id: int):
    try:
        ...
        prompt = (f"Generate a 2-3 sentence motivational message in {lang} "
                  ...
                  f"Start the message with their name and use 2-3 relevant emojis.")
        ...
        await bot.send_message(
            ...
            f"{quote}\n"
            f"══════════════\n\n"
            f"⏰ Daily reminders at 9 PM ({user_data.get('timezone', 'UTC')})",
...
async def process_language_selection(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        await callback.message.answer(t("error_processing", "en"))
        await state.clear()
...
async def process_goal_selection(callback: types.CallbackQuery, state: FSMContext):
    try:
        ...
        await bot.send_message(
            user_id,
            f"✅ {t('registration_complete', lang)}\n"
            f"▫️ {t('edit_goal', lang)}: {goal_text}\n\n"
        ...
        await callback.message.answer(f"❌ {t('error_processing', 'en')}")
        await state.clear()
    await callback.answer()
...
async def log_water(message: types.Message):
    try:
        ...
        logger.error(f"Water logging error for user {message.from_user.id}: {e}")
...
async def callback_log_water(callback_query: types.CallbackQuery):
    try:
        ...
        logger.error(f"Water logging error from callback for user {callback_query.from_user.id}: {e}")
...
async def show_stats(message: types.Message):
    try:
        ...
        logger.info(f"User {user_id} stats: total_meals={total_meals}, today_meals={today_meals}, total_calories={total_calories}, today_calories={today_calories}")
...
async def process_new_language_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(t("error_processing", 'en'), reply_markup=None)
...
async def handle_reminder_toggle(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "toggle_water":
        ...
        current = get_user_ref(user_id).get().to_dict().get('motivational_quotes_enabled', True)
        get_user_ref(user_id).update({'motivational_quotes_enabled': not current})
        status = t("reminder_disabled" if current else "reminder_enabled", lang)
        await callback.answer(f"Motivational quotes {status}")
    elif action == "toggle_breakfast":
        current = get_user_ref(user_id).get().to_dict().get('breakfast_reminder_enabled', True)
        get_user_ref(user_id).update({'breakfast_reminder_enabled': not current})
        status = t("reminder_disabled" if current else "reminder_enabled", lang)
        await callback.answer(f"Breakfast reminder {status}")
    elif action == "toggle_lunch":
        current = get_user_ref(user_id).get().to_dict().get('lunch_reminder_enabled', True)
        get_user_ref(user_id).update({'lunch_reminder_enabled': not current})
        status = t("reminder_disabled" if current else "reminder_enabled", lang)
        await callback.answer(f"Lunch reminder {status}")
    elif action == "toggle_dinner":
        current = get_user_ref(user_id).get().to_dict().get('dinner_reminder_enabled', True)
        get_user_ref(user_id).update({'dinner_reminder_enabled': not current})
        status = t("reminder_disabled" if current else "reminder_enabled", lang)
    ...
    if action in ["toggle_water", "toggle_meal", "toggle_motivation", "toggle_breakfast", "toggle_lunch", "toggle_dinner"]:
        await schedule_default_reminders(user_id, get_user_ref(user_id).get().to_dict().get('timezone', 'UTC'))
...
async def handle_photo(message: types.Message, state: FSMContext = None):
    ...
    try:
        ...
        disclaimer = TRANSLATIONS["disclaimer"][lang]
        ...
        prompt += f"⚠️ Note: {disclaimer}\n\n"
        prompt += "Please strictly follow this format and do not include any additional text."
        logger.info(f"Prompt sent to Gemini for user {user_id}: {prompt}")
...
async def process_meal_text(message: types.Message, state: FSMContext):
    ...
    try:
        if message.photo:
            return await handle_photo_in_text_state(message, state)
...
@dp.callback_query(MealLogging.selecting_type)
